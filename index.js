...
con.connect(function (err) {
  ...
  con.query(sql, function (err, result) {
    if (err) {
      ...
      const sql2 = `INSERT INTO products (name, category, price, stock) VALUES ('Laptop', 'Electronics', 1000, 5)`;
      con.query(sql2, function (err, result) {
        if (err) throw err;
        const sql3 = `INSERT INTO products (name, category, price, stock) VALUES ('Phone', 'Electronics', 500, 10)`;
        con.query(sql3, function (err, result) {
          if (err) throw err;
          console.log("Table and data created");
        });
      });
    }
  });
  ...
  */
  ...
  if (
    ...
    if (!req.body.category) {
      ...
      categoryinfo = `'${req.body.category}'`;
    }
    if (!req.body.price) {
      req.body.price = null;
    }
    ...
    const sql = `INSERT INTO products (name, category, price, stock) VALUES (${nameinfo}, ${categoryinfo}, ${req.body.price}, ${req.body.stock})`;
    con.query(sql, function (err, result) {
      if (err) throw err;
...
app.put("/products/:id", (req, res) => {
  ...
  con.query(sql1, function (err1, result1) {
    if (err1) throw err1;
    if (!result1[0]) return res.status(404).send("Product not found");
    console.log(`Show product id ${req.params.id} information`);
    ...
    if (req.body.category) {
      if (req.body.name == null) {
        result1[0].category = null;
      } else {
        result1[0].category = `'${req.body.category}'`;
      }
    } else {
      result1[0].category = `'${result1[0].category}'`;
    }
    if (req.body.price) {
